...
$OU = "OU=Test OU,OU=Disabled Users,DC=fortunefish,DC=com"
...
foreach ($User in $Users) {
    ...
    if (Get-ADUser -F {SamAccountName -eq $SAM}) {
        ...
        try {
            ...
            if(User.ProxyAddresses.Length -gt 0) {
            ...
            Write-Host "User $SAM imported sucessfully" -BackgroundColor Green
        }
        catch {
            Write-Warning "Something went wrong importing user $SAM."
        }
